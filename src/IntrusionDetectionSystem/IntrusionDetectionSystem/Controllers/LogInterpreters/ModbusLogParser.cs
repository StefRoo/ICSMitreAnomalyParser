...
namespace IntrusionDetectionSystem.Controllers
{
    public class ModbusLogParser : ILogParser
    {
        ...
        {
            return filePath.EndsWith("modbus.log");
        }

        public IEnumerable<DataLine> ParseDataLines(IEnumerable<string> json)
        {
            var dataLines = new List<DataLine>();
            var serializer = new DataContractJsonSerializer(typeof(ModbusDataLine));

            foreach (var line in json)
            {
                using (var ms = new MemoryStream(Encoding.UTF8.GetBytes(line)))
                {
                    if (serializer.ReadObject(ms) is ModbusDataLine parsedLine)
                    {
